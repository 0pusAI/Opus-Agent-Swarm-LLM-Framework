...
The high-level LLM client every agent calls. Owns budget tracking and
...
To run the swarm on a different backend, pass a provider explicitly:

    from opus import LLMClient
    from opus.llm.providers import OpenAIProvider, OllamaProvider, auto_provider
...
Default model names per role are read from the provider (so the Hive
...
class LLMClient:
    """The async LLM façade every agent calls.
    ...
    async def complete(
        ...
        *,
        ...
        apply — the agent layer never has to branch on provider type.
        """
