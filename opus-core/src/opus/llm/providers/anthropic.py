"""
opus.llm.providers.anthropic — the Anthropic provider.

Refactored from the original `opus.llm.client.LLMClient` so the
existing Anthropic-specific tuning (adaptive thinking, prompt caching,
output_config.effort, always-stream-then-get_final_message to dodge
non-streaming HTTP timeouts on long Opus 4.7 outputs) lives in one
provider rather than at the top-level client.
...
#: USD per 1M tokens. Refresh PRICING_AS_OF in opus.provenance when prices change.
...
class AnthropicProvider(LLMProvider):
    """Production-grade Anthropic backend. The original OPUS client."""
...
def _build_system(text: str, *, cache: bool) -> list[dict[str, Any]] | str:
    ...
    on the system text caches `tools + system` together, so every agent
