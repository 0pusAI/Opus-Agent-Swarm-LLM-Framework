"""
opus.llm.providers.base — the provider protocol.

Every LLM backend (Anthropic, OpenAI, Ollama, local, mock) implements
the same surface. This is what makes OPUS a true multi-provider swarm:
one architecture, any model.

Design rules:
  - One `complete()` method. Each provider is responsible for mapping
    OPUS's high-level kwargs to its native API.
  ...
  - Every provider exposes `default_models` for the four canonical roles
    (worker / scout / judge / verifier). The Hive picks sensible defaults
    automatically.
...
class LLMProvider(ABC):
    ...
    def default_models(self) -> dict[str, str]:
        ...
        Override per provider to surface sensible defaults so the Hive
