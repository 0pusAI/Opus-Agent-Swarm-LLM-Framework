"""
opus.llm.providers.mock — deterministic mock provider for tests.

Lets the whole OPUS test suite exercise Hive / agents / consensus
without burning a single API dollar. Two modes:
  ...
  - Templated — pass a callable that gets `(role_signal, system, user)`
    and returns the response text. Useful for asserting agent-specific
    behaviour.

Records every call it received so tests can verify the swarm sent
the right prompts.
"""
