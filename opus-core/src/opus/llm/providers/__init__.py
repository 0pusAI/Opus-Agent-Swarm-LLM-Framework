"""
opus.llm.providers — the multi-provider LLM abstraction.
...
Quickstart with any provider:
    ...
    >>> from opus.llm.providers import OpenAIProvider
    >>> llm = LLMClient(provider=OpenAIProvider())
    >>> hive = Hive(llm=llm)
    >>> result = await hive.run("...")

Or let OPUS pick a provider based on which API key is in the env:

    >>> from opus.llm.providers import auto_provider
...
def auto_provider(name: ProviderName | None = None) -> LLMProvider:
    ...
    Resolution order (when ``name`` is None):
      ...
      1. ``OPUS_PROVIDER`` env var ("anthropic" | "openai" | "ollama") wins.
    ...
    """
    chosen = name or os.environ.get("OPUS_PROVIDER")
