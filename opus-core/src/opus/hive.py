...
The Hive Core: orchestrates one query end-to-end.

Lifecycle (whitepaper §6):

    spawn_scouts ─▶ workers (rounds) ─▶ consensus ─▶ verify
                                                       │
                                falsified ──┐          │
                                            ▼          │
                                  re-deliberate ─ loop ┘
...
class Hive:
    """One Hive per query. Owns the Blackboard for the duration."""
    ...
    async def run(
        ...
        *,
        ...
        scout_task = Task(
            query=query,
            instructions=(
                "Bring back ONE distinctive observation. Other Scouts are "
                "running in parallel; pick an angle they are unlikely to take."
            ),
            budget=budget,
        )
        scouts = [
            ScoutAgent(agent_id=f"scout_{i:02d}", llm=self.llm)
        ...
        # ─── 2. Worker rounds with verification loop ─────────────────
        verifier = VerifierAgent(agent_id="verifier_00", llm=self.llm)
        judge = JudgeAgent(agent_id="judge_00", llm=self.llm)
    ...
    async def _run_worker_round(
        ...
        researchers = [
            ResearcherAgent(agent_id=f"researcher_{round_label}_{i:02d}", llm=self.llm)
        ...
        critics = [
            CriticAgent(agent_id=f"critic_{round_label}_{i:02d}", llm=self.llm)
        ...
        # Synthesisers — parallel
        synth_task = Task(
            query=query,
            instructions=(
                "Integrate the Blackboard into ONE coherent candidate "
                "answer. Accommodate the critiques. Be specific enough "
                "that the Verifier can attempt to falsify your synthesis."
            ),
            budget=budget,
        )
        synthesisers = [
            SynthesiserAgent(agent_id=f"synthesiser_{round_label}_{i:02d}", llm=self.llm)
    ...
    def _build_rankings(self, syntheses: list[Record]) -> list[Ranking]:
        # v0: each Synthesis "votes for itself first" then orders the rest
        # by self-reported confidence. Future: have Critics + Researchers
        # also produce explicit rankings, weighted in Borda.
        ...
        for s in syntheses:
            ...
            rankings.append(
                Ranking(
                    voter_role=AgentRole.SYNTHESISER,
                    voter_id=s.agent_id,
