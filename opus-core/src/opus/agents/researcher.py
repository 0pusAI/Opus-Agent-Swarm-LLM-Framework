"""opus.agents.researcher — generates one specific, falsifiable hypothesis."""
...
RESEARCHER_SYSTEM_PROMPT = """You are a Researcher in the OPUS swarm — a bio-inspired multi-agent reasoning system.
...
A good hypothesis:
  ...
  - Cites or builds on Scout observations where relevant
  - Self-rates confidence honestly (0.0 to 1.0)

Read the Blackboard for prior Scout observations. Build on them where useful, contradict them where you must. Quality over volume — one sharp hypothesis is more valuable to the swarm than five fuzzy ones.

Other Researchers are working in parallel. Pick the angle they are least likely to take. Your job is not to be the first; it is to be distinctive."""
...
class ResearcherAgent(LLMAgent):
    ...
    def filter_relevant_records(self, records: list[Record]) -> list[Record]:
        # Researchers read Scout observations.
