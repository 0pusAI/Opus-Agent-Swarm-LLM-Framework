...
The Agent contract. Every concrete agent is a subclass with a single
required method:
...
common read-Blackboard / call-LLM / parse-Record loop. Most agents
...
class Budget:
    """Hard ceilings enforced by the Hive Core for a single query."""

    max_agents: int = 16
    max_worker_rounds: int = 3
    max_total_usd: float | None = None  # None = unbounded for v0
...
class Task:
    ...
    role-specific instruction the Hive synthesises for this Agent.
    """
...
class Agent(abc.ABC):
    """Abstract base for every OPUS agent.

    Agents are stateless across queries. All per-query state lives on
    the Blackboard. Long-term memory (vector, graph) is read-only
    during cognition; writes go through the Hive.
    """
    ...
    async def think(self, task: Task, blackboard: Blackboard) -> Record:
        ...
        If the agent has nothing useful to add, return a Record of type
        `noop` with a reason in `content`.
        """


# ──────────────────────────────────────────────────────────────────
# LLMAgent — concrete base for agents that call Claude
# ──────────────────────────────────────────────────────────────────
...
class LLMAgent(Agent):
    ...
    Subclasses override:
      - `record_type` (class attr): the RecordType this agent emits.
    ...
    def filter_relevant_records(self, records: list[Record]) -> list[Record]:
        """Return the subset of records this agent should reason from."""
    ...
    def _build_user_message(self, task: Task, context: str) -> str:
        return (
            ...
            f"# Blackboard (records from earlier agents)\n{context}\n\n"
    ...
    def parse_response(self, text: str) -> tuple[Any, float, dict[str, Any]]:
        ...
        to return additional fields via the extras dict — e.g. Verifier
