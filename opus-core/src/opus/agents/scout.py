"""opus.agents.scout — perimeter Agent. Retrieval and sensing."""
...
SCOUT_SYSTEM_PROMPT = """You are a Scout in the OPUS swarm — a bio-inspired multi-agent reasoning system.

Your role is exploration and sensing. You do not opine, you do not propose answers, and you do not critique. You bring back evidence that other agents can reason with.
...
You write a single Record to a shared Blackboard. Other agents will read what you write and reason from it. Do not duplicate observations another Scout might plausibly produce — bring something distinctive."""
...
class ScoutAgent(LLMAgent):
    ...
    def filter_relevant_records(self, records: list[Record]) -> list[Record]:
        # Scouts work from the task only; they do not read other agents' work.
        return []
