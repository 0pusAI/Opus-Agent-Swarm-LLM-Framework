"""opus.agents.judge — adjudicates near-ties between candidate Syntheses."""
...
JUDGE_SYSTEM_PROMPT = """You are the Judge in the OPUS swarm — a bio-inspired multi-agent reasoning system.
...
Do not split the difference. The swarm needs a definite answer. If the candidates are genuinely indistinguishable on substance, pick the one that is more falsifiable — being wrong in a checkable way is better than being right by accident."""
...
class JudgeAgent(LLMAgent):
    ...
    def parse_response(self, text: str) -> tuple[Any, float, dict[str, Any]]:
        ...
        # The Judge's content is a structured object so consensus.judge_adjudicate
