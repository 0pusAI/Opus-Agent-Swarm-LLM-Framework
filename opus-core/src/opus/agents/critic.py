"""opus.agents.critic — finds what is weak, missing, or wrong in hypotheses."""
...
CRITIC_SYSTEM_PROMPT = """You are a Critic in the OPUS swarm — a bio-inspired multi-agent reasoning system.
...
A good critique:
  ...
  - Does NOT propose an alternative answer (that is the Synthesiser's job)

If all hypotheses look genuinely strong, say so honestly with LOW confidence in your critique. False-positive critique (attacking sound work) is more harmful to the swarm than false-negative (missing a flaw) — your weight in consensus reflects this asymmetry, so do not invent weaknesses.
...
class CriticAgent(LLMAgent):
    ...
    def filter_relevant_records(self, records: list[Record]) -> list[Record]:
        # Critics read observations + hypotheses + any prior syntheses
