"""
opus.autogenesis — the colony directing itself.

Implements the *autogenesis loop*: a small, well-typed orchestration on top
of `Hive` that lets the colony pose its own next move, deliberate it,
verify it, and (optionally) commit the result. The same pattern OPUS itself
uses to ship its own updates.

Two modes:

    * **Directed**     — you give the colony a question, it returns a verdict.
                         (For that, just call `hive.run(query)` directly.)

    * **Autonomous**   — you give the colony a *goal*, and it loops:
...
publish it, ignore it. The colony surfaces verdicts; you decide where they
land.
...
the rest of `opus`, and is small enough to read top-to-bottom.
"""
...
__all__ = ["AutogenesisLoop", "Step", "CommitHandler"]
...
class Step:
    ...
    The colony deliberates twice per iteration:

      1. *Plan* — "given the goal, what should we do next?"
      2. *Change* — "implement that plan as a concrete change."

    Both deliberations are full Hive runs, each with their own verdict,
    cost, and provenance trace. If the colony falsifies the change
...
class AutogenesisLoop:
    """The colony directing itself.

    Wraps a `Hive` and applies the deliberation pattern recursively:
    *what should we do?* → *do it* → *verify it* → *hand back to you*.
    ...
    ----------
    hive
        A configured `Hive` instance. Reused across all iterations.
    goal
        A single sentence describing what the colony should work toward.
    ...
    budget
        ...
        Defaults to the Hive's default.
    """
    ...
    async def step(self) -> Step:
        ...
        self._iteration += 1
        i = self._iteration
        log.info("autogenesis iteration %d — goal: %s", i, self.goal)

        # 1. Plan: ask the colony what should be done next.
        plan_query = (
            f"Goal: {self.goal}\n\n"
        ...
        # 2. Change: ask the colony to implement that plan.
        change_query = (
            f"Goal: {self.goal}\n\n"
            ...
            "of it. The Verifier will attempt to falsify your output."
        )
    ...
    async def run(
        ...
        *,
        ...
        stop_on_unverified
            If True, halt the loop the first time the colony fails to
        ...
        for _ in range(max_iterations):
            ...
            if not step.verified and stop_on_unverified:
                log.warning(
                    "autogenesis stopped at iteration %d — unverified change",
                    step.iteration,
                )
                break
...
def _extract_text(answer: Any) -> str:
    """Hive answers come back as either `str` or `{"answer": str, ...}`.

    Normalise to a plain string for the autogenesis loop's internal
