...
Three independently testable functions:
    ...
    judge_adjudicate  — calls a Judge agent on near-ties
    verify            — calls a Verifier agent to attempt falsification
...
# Default v0 weights. Documented in whitepaper §4.1.
# Critics and Verifiers weighted higher because their failure mode
# (false negatives) is less harmful than a Synthesiser's
# (false positives).
DEFAULT_ROLE_WEIGHTS: dict[AgentRole, float] = {
    AgentRole.RESEARCHER:  1.0,
    AgentRole.CRITIC:      1.2,
    AgentRole.SYNTHESISER: 1.5,
    AgentRole.VERIFIER:    1.3,
    AgentRole.PLANNER:     1.0,
    AgentRole.EXECUTOR:    0.8,
}

# Borda-tie threshold: if top two candidates are within EPSILON of each
# other, the Judge fires. See whitepaper §4.1.
EPSILON_TIE = 0.05


@dataclass(frozen=True)
class Ranking:
    """A single Worker's ranked list of candidate Synthesis IDs."""
...
async def judge_adjudicate(
    ...
    """Have the Judge pick a winner from near-tied candidates.

    The Judge sees both candidates AND the trace that produced them.
    ...
    if chosen_id:
        for sc in top:
            if str(sc.record.id) == str(chosen_id):
                return sc
    # Fallback: trust Borda's leader if Judge gave no parseable choice.
    return top[0]
...
async def verify(
    ...
    """Run the Verifier and return either acceptance or a falsification."""
