"""
opus.introspection — the colony reading itself.

The functional bridge between OPUS and its own repository. Tools that
let the autogenesis loop *see* the project it is working on: scan the
filesystem, identify gaps, surface candidate bottlenecks, and feed
them back into the Hive so the colony can deliberate over what to
work on next.
...
doc — it goes back into the colony, and the colony decides which
item is the highest-conviction next move.

Typical use, alongside `AutogenesisLoop`:
    ...
    >>>
    >>> # Feed them into the colony to pick what to work on next.
    >>> verdict = await surface_bottlenecks(hive, observations, n=3)
    >>>
    >>> # The verdict text can become the goal for an AutogenesisLoop.
...
class Observation:
    ...
    The colony reads these as structured context when surfacing the
    next bottleneck.
    """
...
class RepoAnalyst:
    """Walks a repository surfacing things the colony might want to fix.
...
# ──────────────────────────────────────────────────────────────────
# Surfacing the next bottleneck to the colony
# ──────────────────────────────────────────────────────────────────
...
async def surface_bottlenecks(
    ...
    *,
    ...
    """Hand the analyst's observations to the colony for a verdict.

    The colony reads the structured observations as context, deliberates,
    and returns its verdict on which `n` are the highest-conviction next
    moves. The verdict text is suitable to use as the goal for an
    `AutogenesisLoop`.

    If `observations` is empty, the colony is asked to propose a goal
    ...
    observations
        ...
        `max_observations` before being sent to the colony to keep
        context lengths reasonable.
    n
        How many top candidates the colony should return.
    budget
    ...
    query = (
        "The colony just scanned its own repository and surfaced the following "
