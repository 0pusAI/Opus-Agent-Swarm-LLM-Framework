"""
opus.server.ui — the single-page web UI for the local OPUS server.

One HTML file, inline CSS and JS, zero build step, zero external assets.
Embedded as a Python string so `opus serve` ships entirely as a Python
install and works offline once the package is on disk.

Visual language matches the OPUS register exactly: deep matte black
...
<head>
  <meta charset="utf-8">
  <title>OPUS · Local</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="description" content="Your local OPUS colony. Open-source, multi-provider, fully yours.">
...
<header class="nav">
  <div class="brand">
    <!-- Tiny armillary sphere — the OPUS brand mark -->
    ...
    <span class="brand-label">O P U S</span>
  </div>
  <div class="nav-meta">
...
<section class="hero">
  <p class="eyebrow">§ Your Local Colony · MMXXVI</p>
  <h1 class="title">P O S E&nbsp;&nbsp;A&nbsp;&nbsp;Q U E S T I O N</h1>
  <p class="tagline">The full nine-agent OPUS swarm, running on your own machine, on your own keys.</p>
</section>

<form class="compose" id="compose" autocomplete="off">
  <p class="compose-label">— Question for the colony —</p>
  <div class="compose-row">
    <textarea id="query-input"
              placeholder="What is the strongest argument against my own thesis?"
              required></textarea>
    <button type="submit" id="submit-btn">Pose</button>
  </div>
</form>
...
<div class="empty" id="empty">The transcript appears here once the swarm wakes.</div>
<div id="verdict-mount"></div>

<footer>
  <span>OPUS · MMXXVI</span>
  <span>
    <a href="https://github.com/0pusAI/Opus-Agent-Swarm-LLM-Framework" target="_blank" rel="noreferrer">GitHub</a>
    &nbsp;·&nbsp;
    <a href="https://www.tryopusai.com" target="_blank" rel="noreferrer">tryopusai.com</a>
  </span>
</footer>
...
(function () {
  ...
  function recordCard(rec) {
    ...
    if (rec.agent_role === 'verifier') card.classList.add('is-verifier');
    if (rec.agent_role === 'judge')    card.classList.add('is-judge');
  ...
  function showVerdict(payload) {
    ...
    title.textContent = payload.accepted ? '— Verdict · verified ✓ —' : '— Verdict · unresolved falsifications —';
  ...
  form.addEventListener('submit', async (ev) => {
    ...
    setPhase('deliberating', 'Scouts gathering…');
