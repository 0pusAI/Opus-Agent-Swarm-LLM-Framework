"""
opus.server — the local web UI and HTTP API.

Boots a tiny FastAPI app that:

  * Serves a single-page web UI at ``/`` matching OPUS's brand register.
...
Boot it from a script:
    ...
    from opus import auto_provider, LLMClient
    from opus.server import create_app, run_server
