...
The smallest possible end-to-end OPUS run. Requires ANTHROPIC_API_KEY
...
This script also runs as `opus query "..."` once installed — see cli.py.
hello_swarm exists so the README's "smallest example" is self-contained.
"""
...
# Allow `python examples/hello_swarm.py` from the opus-core dir without install.
...
from opus.blackboard import RecordType  # noqa: E402
...
async def main() -> None:
    ...
    console = Console()
    console.print(Panel(
        f"[bold cyan]OPUS — hello_swarm[/bold cyan]\n\n{QUERY}",
        title="Query",
        border_style="cyan",
    ))
    ...
    try:
        ...
        result = await hive.run(QUERY, budget=Budget())
    finally:
        await llm.aclose()
    ...
    console.print(Panel(
        rendered,
        title=("[green]Final Answer (verified)[/green]" if result.accepted
               else "[yellow]Final Answer (unresolved falsifications)[/yellow]"),
