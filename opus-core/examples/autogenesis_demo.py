"""
examples/autogenesis_demo.py

A minimal end-to-end demonstration of `opus.autogenesis.AutogenesisLoop`.

The colony is handed a *goal* (not a question) and runs the
...
in Slack, send it to a queue. The colony surfaces verdicts; you decide
where they land.
...
Usage:
    ...
    python examples/autogenesis_demo.py

Cost: approximately $1–$3 per iteration (two full Hive deliberations
...
# Allow `python examples/autogenesis_demo.py` from the opus-core dir
# without an install.
...
GOAL = (
    "Improve the documentation of opus-core for first-time users. "
...
async def main() -> None:
    ...
    console = Console()
    console.print(Panel(
        f"[bold cyan]OPUS — autogenesis demo[/bold cyan]\n\n"
        f"[bold]Goal:[/bold] {GOAL}\n"
        f"[bold]Max iterations:[/bold] {MAX_ITERATIONS}",
        title="Autogenesis Loop",
        border_style="cyan",
    ))
    ...
    try:
        ...
        # verified step. Here we just print it. In a real autogenesis
