...
from opus.blackboard import AgentRole, Record, RecordType
from opus.consensus import (
...
def _synth(agent_id: str, confidence: float = 0.8) -> Record:
    return Record(
        agent_id=agent_id,
        agent_role=AgentRole.SYNTHESISER,
...
def test_borda_single_voter_picks_first_ranked():
    ...
    # 2 candidates: first gets 2 pts, second gets 1; weight 1.5 for synth.
    assert scored[0].score == 2 * DEFAULT_ROLE_WEIGHTS[AgentRole.SYNTHESISER]
    assert scored[1].score == 1 * DEFAULT_ROLE_WEIGHTS[AgentRole.SYNTHESISER]
...
def test_borda_aggregates_across_voters():
    ...
    rankings = [
        Ranking(AgentRole.RESEARCHER, "v1", (a.id, b.id, c.id)),
        Ranking(AgentRole.CRITIC,     "v2", (b.id, a.id, c.id)),
    ]
...
def test_borda_ignores_unknown_candidates_in_rankings():
    ...
    bogus_id = uuid4()
    rankings = [
        Ranking(AgentRole.SYNTHESISER, "v1", (a.id, bogus_id)),
    ]
...
def test_is_near_tie_true_when_within_epsilon():
    from opus.consensus import ScoredCandidate
    a = _synth("a")
    b = _synth("b")
    scored = [
