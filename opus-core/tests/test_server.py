"""Tests for opus.server — the local web UI + HTTP API.
...
from opus.llm.providers import MockProvider
from opus.server import create_app


# A canned mock response shaped like what the agents expect to parse.
_AGENT_JSON_RESPONSE = json.dumps({
    "content": "A short verdict from the mock swarm.",
    "confidence": 0.8,
})
...
def mock_llm() -> LLMClient:
    ...
    # Same response shape for every agent so the Hive can parse, vote,
...
def client(mock_llm: LLMClient) -> httpx.AsyncClient:
    ...
    app = create_app(
        mock_llm,
        # Smallest possible Hive so tests stay fast.
        hive_kwargs={
            "n_scouts": 1,
            "n_researchers": 1,
            "n_critics": 1,
...
async def test_index_returns_html(client: httpx.AsyncClient):
    ...
    assert "OPUS" in body
    assert "Your Local Colony" in body
