"""Tests for opus.llm.providers — the multi-provider abstraction.

No live API calls. Anthropic / OpenAI / Ollama paths are exercised
against an httpx MockTransport so the swarm tests stay deterministic,
...
from opus.llm.providers import (
    AnthropicProvider,
    CompletionResult,
    MockProvider,
    OllamaProvider,
...
from opus.llm.providers.openai import PRICING_USD_PER_MTOK as OPENAI_PRICES
from opus.provenance import TokenUsage
...
def _clear_provider_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for key in ("OPUS_PROVIDER", "ANTHROPIC_API_KEY", "OPENAI_API_KEY", "OLLAMA_HOST"):
...
def test_auto_provider_honours_opus_provider_env(monkeypatch: pytest.MonkeyPatch):
    """OPUS_PROVIDER env var takes precedence over auto-detection."""
    _clear_provider_env(monkeypatch)
    monkeypatch.setenv("OPUS_PROVIDER", "openai")
    monkeypatch.setenv("OPENAI_API_KEY", "test")
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test")
    provider = auto_provider()
    assert isinstance(provider, OpenAIProvider)
