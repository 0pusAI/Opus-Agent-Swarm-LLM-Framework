...
async def test_read_filters_by_role():
    bb = InMemoryBlackboard()
    await bb.append(_record(role=AgentRole.SCOUT, content="s"))
    await bb.append(_record(role=AgentRole.RESEARCHER, content="r"))

    scouts = await bb.read(roles=[AgentRole.SCOUT])
    assert len(scouts) == 1
