"""Tests for opus.introspection — Observation, RepoAnalyst, surface_bottlenecks.
...
`surface_bottlenecks` async helper is tested against a stub Hive that
...
from opus.introspection import (
    Observation,
    RepoAnalyst,
    surface_bottlenecks,
)
...
# ──────────────────────────────────────────────────────────────────
# surface_bottlenecks — async, stubbed Hive
# ──────────────────────────────────────────────────────────────────
...
async def test_surface_bottlenecks_with_observations():
    ...
    result = await surface_bottlenecks(hive, obs, n=2)  # type: ignore[arg-type]
    assert result.answer == "stub verdict"
    assert len(hive.received_queries) == 1
    query = hive.received_queries[0]
...
async def test_surface_bottlenecks_empty_observations_asks_from_scratch():
    ...
    hive = _StubHive()
    result = await surface_bottlenecks(hive, [], n=3)  # type: ignore[arg-type]
    assert result.answer == "stub verdict"
    assert len(hive.received_queries) == 1
    # The from-scratch prompt should NOT include the structured-observations text.
    assert "no surfaced gaps" in hive.received_queries[0]
...
async def test_surface_bottlenecks_caps_observation_count():
    ...
    await surface_bottlenecks(hive, many, n=3, max_observations=5)  # type: ignore[arg-type]
    query = hive.received_queries[0]
